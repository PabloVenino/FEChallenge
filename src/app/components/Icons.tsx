// Small inline icon set shared across the copilot UI.
// Kept as simple SVG wrappers so callers can pass className/size without
// duplicating the same <svg> boilerplate everywhere.
...
export function ErrorIcon({ className, size = 14 }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={className}>
...
export function ToolSpinnerIcon({ className, size = 14 }: IconProps) {
  return (
    <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className={`animate-spin ${className ?? ""}`}>
      <path d="M21 12a9 9 0 1 1-6.219-8.56" />
    </svg>
  );
}

export function EmptyPipelineIcon({ className, size = 32 }: IconProps) {
  return (
